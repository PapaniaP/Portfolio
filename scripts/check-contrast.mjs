...
const variants = readFileSync(new URL('../src/styles/palette-variants.css', import.meta.url), 'utf8');
const luminance = (hex) => {
...
for (const theme of ['dark', 'light']) {
 for (const palette of ['base', 'teal', 'blue', 'violet']) {
  const block = css.match(new RegExp(`:root\\[data-theme=${theme}\\]\\s*\\{([^}]+)`))[1];
  const overrides = palette === 'base' ? '' : variants.match(new RegExp(`:root\\[data-palette=${palette}\\]\\[data-theme=${theme}\\]\\s*\\{([^}]+)`))[1];
  const tokens = Object.fromEntries([...(block + overrides).matchAll(/--color-([\w-]+):\s*(#[a-f\d]{6})/gi)].map(match => [match[1], match[2]]));
